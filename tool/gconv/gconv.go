...
//float64 保留n位小数, 四舍六入五考虑，五后非零就进一，五后为零看奇偶，五前为偶应舍去，五前为奇要进一
func Round64Str(i interface{}, num int) string {
	formt := "%." + strconv.Itoa(num) + "f"
	v := Float64(i)
	return fmt.Sprintf(formt, v)
}
